...
# 结构已经改了下，两个service其实可以合并写在一起，不用分开
...
对文件中keywords进行排序（那样读取的10min要优化 - 并行读取） - 可以先生成索引，然后按照索引重新将保存的文件一个个读取出来然后重新生成一个keywords有序的文件   多用并行读取
...
***grpc的使用如果官方文档不够，或许可以看看真实项目代码是怎么用异步接口等的***
***grpc官方仓库很多examples可供学习！***

## 目前提交分析
提交id 59398 分数 49.579 \
就是目前的这种架构 balancer - 三个Search Server - 三个SearchLocal Server；读取csv建立索引的时间是8分钟(跟10分钟很接近！最好优化下) \
提交id 60020 分数 49.589 \
上一个提交中删去了Data结构中两个无用的u64作用域，看样子没有提升
