  ...
  void Search(const std::vector<uint64_t> &keywords,
    ...
    if (status.ok()) {
      for (int i = 0; i < response.adgroup_ids_size(); i++) {
        // std::cout << "Adgroup ID: " << response.adgroup_ids(i) << ", Price: "
        // << response.prices(i) << std::endl;
      }
    } else {
...
int main(int argc, char **argv) {
  ...
  // std::string server_address("172.17.0.2:56789");
  ...
  while (true) {
    ...
    std::cout << "QPS: " << qps << ", Average Latency: " << avg_latency
              << " ms\n";
  }
